...
export const getData = async (req, res) => {
	try {
		...
		if (trafficLightValue.length == 0) {
			trafficLightValue.push({ value: null });
		}
		if (lightValue.length == 0) {
			lightValue.push({ value: null });
		}
		if (DHT11Value.length == 0) {
			lightValue.push({ value: null, value2: null });
		}

		const dataMessage = {
			condition: trafficLightValue[trafficLightValue.length - 1]?.value,
			temperature: DHT11Value[DHT11Value.length - 1]?.value,
			humidity: DHT11Value[DHT11Value.length - 1]?.value2,
			light: lightValue[lightValue.length - 1]?.value,
		};
...
export const getChartData = async (req, res) => {
	...
	try {
		...
		//===========================================================================================
		let dhtHour =
			String(DHT11[DHT11.length - 1]?.time?.getHours()) < 10
				? '0' + String(DHT11[DHT11.length - 1]?.time?.getHours())
				: String(DHT11[DHT11.length - 1]?.time?.getHours());
		let dhtMinute =
			String(DHT11[DHT11.length - 1]?.time?.getMinutes()) < 10
				? '0' + String(DHT11[DHT11.length - 1]?.time?.getMinutes())
				: String(DHT11[DHT11.length - 1]?.time?.getMinutes());
		let lightHour =
			String(Light[Light.length - 1]?.time?.getHours()) < 10
				? '0' + String(Light[Light.length - 1]?.time?.getHours())
				: String(Light[Light.length - 1]?.time?.getHours());
		let lightMinute =
			String(Light[Light.length - 1]?.time?.getMinutes()) < 10
				? '0' + String(Light[Light.length - 1]?.time?.getMinutes())
				: String(Light[Light.length - 1]?.time?.getMinutes());
