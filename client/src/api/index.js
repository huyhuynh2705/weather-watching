...
API.interceptors.request.use(req => {
  if (localStorage.getItem(TOKEN_NAME)) {
    req.headers.Authorization = `Bearer ${
      JSON.parse(localStorage.getItem(TOKEN_NAME)).token
    }`
  }
  return req
})
...
export const updateProfile = (id, form) =>
  API.post(`/user/updateprofile/${id}`, form)
...
export const updateDeviceSet = form =>
  API.post('/set/admin/updateDeviceSet', form)
...
export const getTrafficlightName = () =>
  API.get('/device/admin/TrafficlightName')
